...
/**
 * Utility methods to copy, clone and convert a stream to a String.
 * 
 * @author tstich
 * 
 */
public class StreamUtils {
	...
	/**
	 * Copies all bytes from an InputStream to an OutputStream. The Bufsize
	...
	/**
	 * Copies all bytes from an InputStream to an OutputStream. The Bufsize is
	 * set to 8000 bytes.
	 * 
	 * @param _isInput
	 *            InputStream from wihch the bytes are to copied.
	...
	public static StringBuffer readInString(InputStream in) throws IOException {
		// Use batch copying instead of single read operations.
		// Patch contributed by Pete Buletza 
		
		StringBuffer strBuf = new StringBuffer(in.available());

		byte[] buf = new byte[BUFFERSIZE];
		int iCharsRead = 0;
		while ((iCharsRead = in.read(buf, 0, BUFFERSIZE)) != -1) {
			for (int j = 0; j < iCharsRead; ++j) {
