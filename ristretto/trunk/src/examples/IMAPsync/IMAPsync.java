...
public class IMAPsync {
	...
	private static final Pattern SERVER_REGEXP = Pattern
			.compile("^([^@]+)@([^:]+):(.*)$");

	private static final String[] HEADERS = new String[] { "Date", "Subject",
			"From", "Message-ID", "Message-Id" };
	...
	public static void main(String[] args) throws IOException, IMAPException {
		...
		if (args[0].startsWith("-")) {
			...
			if (listfolders)
				System.out.println("Listing mailboxes only!");
		...
		IMAPProtocol source = new IMAPProtocol(srcMatcher.group(2),
		...
		IMAPProtocol dest = new IMAPProtocol(destMatcher.group(2),
				IMAPProtocol.DEFAULT_PORT);
		dest.openPort();
		...
		IMAPHeader[] srcHeader = source.uidFetchHeaderFields(SequenceSet
				.getAll(), HEADERS);

		System.out.println("Fetching messagelist from dest...");
		IMAPHeader[] destHeader = dest.uidFetchHeaderFields(SequenceSet
				.getAll(), HEADERS);

		// Computing the diffs

		ArrayList<IMAPHeader> srcList = new ArrayList<IMAPHeader>(Arrays
				.asList(srcHeader));
		ArrayList<IMAPHeader> destList = new ArrayList<IMAPHeader>(Arrays
				.asList(destHeader));
		...
		ListTools.substract(srcList, destList, new Comparator() {
			public int compare(Object arg0, Object arg1) {
				...
				if (a.getMessageID() != null && b.getMessageID() != null
						&& a.getMessageID().equals(b.getMessageID())) {
					return 0;
				}
		...
		for (IMAPHeader h : copyList) {
			...
			if (!dryrun)
				dest.append(destMatcher.group(3), source.uidFetchMessage(h
						.getUid()));
		}
		source.logout();
		dest.logout();
	...
	private static void printUsage() {
		System.out
				.println("IMAPrsync [options] user@src.imap.server:mailbox user@dest.imap.server:mailbox");
