...
public class MailboxNameUTF7Converter
{
	...
	public static String encode(String mailboxName)
	{
		...
		for (int i = 0; i < mailboxName.length(); i++)
		{
			...
			if (isPrintable(c))
			{
				if (mode != PRINTABLE && buffer.position() > 0)
				{
					...
					//System.out.println("vor put oben " + buffer.capacity() + ":" + buffer.remaining());
					if (buffer.position() % 3 != 0)
					{
						buffer.put(new byte[3 - (buffer.position() % 3)]);
					}
					//System.out.println("nach put oben " + buffer.capacity() + ":" + buffer.remaining());
					buffer.limit(buffer.position());
					//System.out.println("nach limit " + buffer.capacity() + ":" + buffer.remaining());
			...
			else
			{
				...
				try
				{
					...
					//System.out.println("vor put " + buffer.capacity() + ":" + buffer.remaining());
					...
					//System.out.println("nach put " + buffer.capacity() + ":" + buffer.remaining());
					if (buffer.remaining() == 0)
					{
						buffer.limit(buffer.limit() + 1);
					}
		...
		//System.out.println("schlußendlich :" + returnString);
		return returnString;
	}
