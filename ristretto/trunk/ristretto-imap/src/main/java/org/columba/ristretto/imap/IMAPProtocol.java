...
public class IMAPProtocol implements AuthenticationServer {
	...
	/**
	 * Puts the client in IDLE mode. If the server
	 * supports the IDLE extension (check the capabilities),
	 * the client is then waiting for getting updates
	 * without the need to poll the server.
	 * <br>
	 * When issuing another command while in IDLE state
	 * an automatic DONE command will be sent. If you
	 * want to stop idling manually call #done();
	 * <br>
	 * Protocol has to be in SELECTED state
	 * 
	 * @see #capability()
	 * @see #done()
	 * 
	 ...
	 * @throws IOException
	 */
	public void idle() throws IMAPException, IOException {		
		checkState(SELECTED);
		state = IDLE;
		IMAPCommand command = new IMAPCommand(tagFactory.nextTag(),"IDLE");
		IMAPResponse response = communicate(command)[0];
		
		if( response.getResponseType() != IMAPResponse.RESPONSE_CONTINUATION) {
			// We can assign the SELECTED state since we checked it earlier
			state = SELECTED;
			handleResponse(response);
		}
	}
	...
	/**
	 * Stops the idling manually.
	 *  
	 * <br>
	 * Protocol has to be in IDLE state
	 * 
	 * @see #idle
	 * 
	 * @throws IMAPException
	 * @throws IOException
	 */
	public void done() throws IMAPException, IOException {
		checkState(IDLE);
		
		IMAPCommand command = new IMAPCommand("DONE");
		IMAPResponse response = communicate(command)[0];
	...
	public IMAPFlags[] fetchFlags(SequenceSet set) throws IOException,
		IMAPException {		
			return fetchFlagsCore("FETCH",set);
	}
	 ...
	 * @throws IMAPException
	 */	
	public IMAPFlags[] uidFetchFlags(SequenceSet set) throws IOException,
	IMAPException {		
		return fetchFlagsCore("UID FETCH",set);
	}
	
	protected IMAPFlags[] fetchFlagsCore(String c, SequenceSet set) throws IOException,
			IMAPException {
		checkState(SELECTED);
	...
	/**
	 * Fetch the Uids of the specified messages.
	 * 
	 * @param set the indices of the messages
	 * @return the Uids of the messages
	 * @throws IOException
	 * @throws IMAPException
	 */
...
public CopyInfo uidCopy(SequenceSet set, String mailbox) throws IOException,
	...
	private synchronized IMAPResponse[] communicate(IMAPCommand command)
		...
		try {
			response = in.readResponse();
			while (!response.isTagged() && !response.isBYE() && state!=IDLE) {
		...
		if (response.isTagged() && command.getTag() != null) {
	...
	private void checkState(int state) throws IMAPException, IOException {
		if (getState() < state) {
		...
		if( getState() == IDLE && state != IDLE) {
			// issue a done command first
			done();
		}
	}
