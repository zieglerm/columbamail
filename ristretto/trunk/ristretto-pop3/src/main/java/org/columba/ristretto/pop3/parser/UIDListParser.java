...
public class UIDListParser
{
    ...
    /**
     * Parse the UidListEntry from the input String.
     * 
     * @param in
     * @return the UidListEntry
     * @throws ParserException
     */
    public static UidListEntry parse( CharSequence in ) throws ParserException {
    	Matcher matcher = uidPattern.matcher(in);
    	if( matcher.find() ) {
    		return new UidListEntry(Integer.parseInt(matcher.group(1)), matcher.group(2));
    	}
    	
    	throw new ParserException( "Could not parse UIDL: "+ in);
    }
