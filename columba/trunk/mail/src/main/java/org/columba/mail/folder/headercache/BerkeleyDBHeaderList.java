...
public class BerkeleyDBHeaderList implements IPersistantHeaderList {
	...
	private synchronized void openEnvironment() throws DatabaseException {
		...
		if( !headercacheDirectory.exists()) headercacheDirectory.mkdir();
		
		EnvironmentConfig environmentConfig = new EnvironmentConfig();
		environmentConfig.setAllowCreate(true);
		environmentConfig.setCachePercent(80);
		// perform other environment configurations
		try {
			environment = new Environment(headercacheDirectory, environmentConfig);			
	...
	public void clear() {
		...
		try {
			environment.truncateDatabase(null, name, true);
		} catch (DatabaseException e) {
