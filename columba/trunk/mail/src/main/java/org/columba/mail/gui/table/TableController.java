...
/**
 * Shows the message list. By default, this is the read/unread state if a message, Subject:, Date:, From: and Size headerfields.
 * <p>
 * Folder-specific configuration options are handled by <code>FolderOptionsController</code> and can be configured by the user
 * in the AbstractMessageFolder Options Dialog.
 * 
 * @author fdietz
 */
public class TableController implements ListSelectionListener, TableModelChangedListener, ITableController {
   ...
   * <p>
   * Tooltips for columns can be found in org.columba.mail.i18n.header.header.properties.
   * 
  ...
  public void createPopupMenu() {
    try {
      InputStream is = DiskIO.getResourceStream("org/columba/mail/action/table_contextmenu.xml");
  ...
  public void tableChanged(TableModelChangedEvent event) {
     ...
     * getFrameController()) .getFolderInfoPanel().setFolder((IMailFolder) srcFolder); } }
     */
    ...
    case -1: {
       ...
       * getView().getTree().getPathForRow(i); MessageNode node = (MessageNode) path .getLastPathComponent(); IColumbaHeader h =
       * node.getHeader(); boolean unseen = !h.getFlags().getSeen(); if (unseen) { getView().getTree().expandPath(path); } } }
       */
      break;
    }
  ...
  public void valueChanged(ListSelectionEvent arg0) {
    ...
    /*
     * // @author fdietz // bug #983931, message jumps while downloading new messages if (getView().getSelectedNodes().length ==
  ...
  class TableHeaderMouseListener extends MouseAdapter {
     ...
     *  
     */
    public TableHeaderMouseListener(TableModelSorter sorter) {
