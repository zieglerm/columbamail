...
class AccountCreator implements WizardModelListener {
	...
	public void wizardFinished(WizardModelEvent e) {
		...
		account.getIdentity().setAddress(
				(Address) data.getData("Identity.address"));
...
//						.createIMAPFolder(parentFolder, "INBOX");
		...
		try {
			...
			WelcomeMessage.addWelcomeMessage(folder, adr.toString(),
					new Integer(account.getUid()).toString());
		} catch (Exception e1) {
			e1.printStackTrace();
		}
