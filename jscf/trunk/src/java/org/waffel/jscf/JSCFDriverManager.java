...
/**
 * This is the basis service to manage a set of JSCF drivers. JSCF Drivers
 * provides functions to sign, verify, encrypt and decrypt Streams.
 * <p>
 * If the <b>JSCFDriverManager </b> start the initialisiation, it attempts to
 * load the driver classes referenced in the "jscf.drivers" system property. A
 * user can customize his JSCF Drivers in his application using the property.
 * For example in your ~/.hotjava/properties file you can specify following:
 * 
 * <pre>
 * <code>
 ...
 * same classloader as the current applet or application. The same clasloader is
 * used with <code>JSCFDriverManager.class.getClassloader</code>. The
 * implementation to use the classloader don't need a native routine as it the
 * JDBC DriverManager does.
 * 
...
public final class JSCFDriverManager {
     ...
     * <code>registerJSCFDriver</code> to make itself known to the
     ...
     * registered driver is ask if it understands the given url. The first
    ...
    // initializes the Class and loads all initial Drivers
